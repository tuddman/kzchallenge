(ns kzchallenge.server
  ...
  (:require [org.httpkit.server       :refer [run-server]]
            ...
            [kzchallenge.handlers        :refer [handler auth-backend]]
...
(defn reload []
  (do
    (stop-server)
    (reset! server (run-server #'app {:port (Integer. (or (env-var "PORT") 8080))}))))
...
(defn -main [& args]
  (let [{:keys [options arguments errors summary]} (parse-opts args cli-options)]
    ...
    (case (first arguments)
      ...
      (->
       (reset! server (run-server #'app {:port (Integer. (or (env-var "PORT") 8080))}))
       (log/info (str "server started. listen on 0.0.0.0:" (or (env-var "PORT") 8080))))
      "stop" (stop-server)
